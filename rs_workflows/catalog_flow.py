...
async def publish(
    ...
    with flow_env.start_span(__name__, "publish-to-catalog"):
        for item in items:
            try:
                # Resolve destination collection using product type from STAC item
                target_collection = resolve_collection(item.properties["product:type"], target_collections)
...
def resolve_collection(product_type: str, target_collections: list[FlowGeneratedProduct]) -> str:
    """Resolve the target collection for a given product type.

    Supports:
    - iterable of FlowGeneratedProduct (new)
