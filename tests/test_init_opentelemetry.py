...
from rs_common.init_opentelemetry import (
    botocore_hook,
    init_traces,
    parse_data,
    requests_hook,
...
def test_start_span_creates_root_span(mocker):
    ...
    get_tracer = mocker.patch("rs_common.init_opentelemetry.trace.get_tracer", return_value=tracer)
...
def test_start_span_creates_child_span_from_parent_context(mocker):
    ...
    tracer = mocker.Mock()
    tracer.start_as_current_span.side_effect = fake_start_as_current_span
    mocker.patch("rs_common.init_opentelemetry.trace.get_tracer", return_value=tracer)
    # Child spans first bind a NonRecordingSpan made from the parent SpanContext.
    use_span = mocker.patch("rs_common.init_opentelemetry.trace.use_span", side_effect=fake_use_span)
...
def test_instrumentation(mocker, monkeypatch):
    ...
    """
    mocker.patch("rs_common.init_opentelemetry.initialized", False)
    monkeypatch.setenv("TEMPO_ENDPOINT", "none")

    mocker.patch("rs_common.init_opentelemetry.auto_instrumentation")
    mocker.patch("opentelemetry.instrumentation.instrumentor.BaseInstrumentor.instrument")

    init_traces(service_name="pytest")
