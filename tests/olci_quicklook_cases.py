...
with patch.dict(
    ...
    # Resolve through sys.modules: restoring patch.dict can leave stale package attributes.
    common = import_module("rs_workflows.on_demand.sentinel3.olci_quicklook_common")
    l1 = import_module("rs_workflows.on_demand.sentinel3.s3_l1_olci_quicklook")
    l2 = import_module("rs_workflows.on_demand.sentinel3.s3_l2_olci_quicklook")
...
class OlciQuicklookTests:
    """Run the same catalog, image and failure scenarios for either OLCI processing level."""

    level = 1

    @pytest.fixture(name="quicklook_context")
    def _quicklook_context(self, mocker, monkeypatch):
        """Keep the image calculations real while replacing runner dependencies and remote I/O."""
        if self.level == 1:
            channels = ("oa08_radiance", "oa06_radiance", "oa04_radiance")
            generate = l1.generate_s3l1_olci_quicklooks.fn
        else:
            channels = ("otci", "gifapar", "iwv")
            generate = l2.generate_s3l2_olci_quicklooks.fn
        collection = f"olci-l{self.level}"
        # Build a small swath with distinct band patterns to check RGB channel mapping and scaling.
        rows, columns = np.indices((9, 8))
        values = (rows * 8 + columns).astype(float)
        measurements = xr.Dataset(
            {
                "longitude": (("rows", "columns"), 10.0 + columns),
                "latitude": (("rows", "columns"), 50.0 - rows),
                channels[0]: (("rows", "columns"), values),
                channels[1]: (("rows", "columns"), -values),
                channels[2]: (("rows", "columns"), columns.astype(float)),
            },
        )
        ...
        return SimpleNamespace(
            ...
            cog_options={} if self.level == 1 else {"nodata": 0},
            reproject_calls=1 if self.level == 1 else 2,
            item=item,
    ...
    async def test_generate_quicklooks(self, quicklook_context, projection_present):
        ...
        result = await ctx.generate(OWNER, ctx.published_items)

        # Check that the catalog lookup and product read use the expected owner, href and credentials.
        ctx.catalog.get_item.assert_called_once_with(ctx.collection, "product", owner_id=OWNER)
        ...
        assert ctx.rasterio.warp.reproject.call_count == ctx.reproject_calls
        warp_args = ctx.rasterio.warp.reproject.call_args_list[0].kwargs
    ...
    async def test_generate_quicklooks_rejects_invalid_items(self, quicklook_context, case, message):
        ...
        ctx = quicklook_context
        published: list[dict[str, Any]] = ctx.published_items
        if case == "empty":
            published = []
        elif case == "missing_id":
            published = [{"collection": ctx.collection}]
        ...
        with pytest.raises(ValueError, match=message):
            await ctx.generate(OWNER, published)
    ...
    async def test_generate_quicklooks_rejects_invalid_measurements(self, quicklook_context, case, message):
        ...
        with pytest.raises(ValueError, match=message):
            await ctx.generate(OWNER, ctx.published_items)
    ...
    async def test_generate_quicklooks_accepts_constant_values(self, quicklook_context):
        """A constant channel becomes zero intensity without preventing publication."""
        ctx = quicklook_context
        ctx.measurements[ctx.red_channel].values[:] = 1
        # Equal percentiles produce NaNs during scaling, which are then converted to zero.
        with np.errstate(invalid="ignore", divide="ignore"):
            await ctx.generate(OWNER, ctx.published_items)

        rgb = ctx.rasterio.warp.reproject.call_args_list[0].kwargs["source"]
        assert np.all(rgb[0] == 0)
        assert ctx.upload.await_count == 2
        ctx.catalog.patch_item.assert_called_once()

    async def test_generate_quicklooks_does_not_patch_after_upload_failure(self, quicklook_context):
